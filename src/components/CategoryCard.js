function CategoryCard({ title, description, icon, color, page }) {
  try {
    const handleClick = (e) => {
      e.preventDefault();
      // 使用 window.location.href 进行快速跳转
      window.location.href = page;
    };

    const handleMouseEnter = () => {
      // 预加载目标页面资源
      if (window.BLOG_CONFIG?.ENABLE_PRELOAD) {
    ...
    return (
      <div className="card-hover group cursor-pointer" data-name="category-card" data-file="components/CategoryCard.js" onClick={handleClick} onMouseEnter={handleMouseEnter}>
