class BlogManagerKV {
  constructor() {
    this.kvManager = new CloudflareKVManager();
  ...
  async initialize() {
    ...
    try {
      const posts = await this.kvManager.get('blogPosts');
      this.posts = posts || this.getDefaultPosts();
  ...
  async savePosts() {
    try {
      const success = await this.kvManager.put('blogPosts', this.posts);
      if (!success) {
        console.error('Failed to save posts to KV storage');
      }
      return success;
    } catch (error) {
  ...
  async exportData() {
    ...
    return {
      ...
      storageInfo: this.kvManager.getStorageInfo()
    };
  }
  ...
  async importData(data) {
    try {
      if (data.posts && Array.isArray(data.posts)) {
        ...
        const success = await this.savePosts();
        return success;
      }
      return false;
    } catch (error) {
  ...
  // 获取存储信息
  getStorageInfo() {
    return this.kvManager.getStorageInfo();
  }
  ...
  async clearAllData() {
    try {
      this.posts = [];
      await this.kvManager.delete('blogPosts');
      return true;
    } catch (error) {
